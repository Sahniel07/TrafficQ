...
def process_pair(args):
    ...
    # Downsample trajectories: take every 2nd point now
    if len(traj1) > 2: # Adjust condition if needed
        traj1 = traj1[::2]
    if len(traj2) > 2: # Adjust condition if needed
        traj2 = traj2[::2]
...
def plot_all_clusters_in_one_image(trajectories, clusters, output_dir, min_lon, max_lon, min_lat, max_lat):
    ...
    # 删除排序，使用原始聚类顺序以保持与单独图像和JSON文件的一致性
    ...
    for i, cluster in enumerate(tqdm(all_clusters, desc="Drawing clusters")):
...
def main():
    ...
    # 使用改进的参数进行聚类
    print("\n=== 使用改进参数进行聚类 ===")
    similarity_threshold = 280.0   # DTW距离阈值(米) - 增大以形成更大簇
    centroid_threshold = 600.0     # 质心预过滤阈值(米) - 保持不变或根据需要调整
    min_samples = 2                # DBSCAN参数:最小样本数 - 保持较低值
    ...
    # 如果需要按照特定顺序排序聚类，我们在这里进行排序
    # 例如，可以按照聚类大小排序，确保大型聚类有较小的ID
    # multi_vehicle_clusters = sorted(multi_vehicle_clusters, key=len, reverse=True)
    ...
    with open("single_vehicles.json", "w", encoding="utf-8") as f:
        single_summary = {
            ...
            "single_vehicles_list": single_output_list_for_summary
        }
